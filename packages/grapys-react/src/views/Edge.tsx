...
const Edge: React.FC<EdgeProps> = ({
  ...
  const edgePath = useMemo(() => {
    ...
    const y1Offset =
      index !== undefined && outputCenters && outputCenters.length >= index
        ? outputCenters[index]
        : 0;
    const y2Offset =
      index2 !== undefined && inputCenters && inputCenters.length >= index2
        ? inputCenters[index2]
        : 0;
    ...
    const controlYOffset =
      (ydashDiff > 40 ? 40 : ydashDiff) * (y1dash > y2dash ? 1 : -1);
    ...
    const controlXOffset =
      xDiff > 0
        ? minOffset + (xDiff > offsetThreshold ? 0 : offsetThreshold - xDiff)
        : maxOffset;
